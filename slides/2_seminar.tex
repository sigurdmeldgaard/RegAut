...
\column{2.5cm}
\onslide<10->{\includegraphics[scale=0.35]{images/2_seminar_min_ex_3}}
\column{3.5cm}
\scalebox{.5}{
\begin{tabular}{c|c|c|c|c|c|c|}
  2 &  \\
  3 & \onslide<7->{X} & \onslide<7->{X}  \\
  4 &   &   & \onslide<7->{X} \\
  5 & \onslide<7->{X} & \onslide<7->{X} &   & \onslide<7->{X} \\
  6 & \onslide<5->{\onslide<5->{X}} & \onslide<5->{\onslide<5->{X}} & \onslide<5->{X} & \onslide<5->{X} & \onslide<5->{X} \\
  7 & \onslide<7->{X} & \onslide<7->{X} &   & \onslide<7->{X} &   & \onslide<7->{X}  \\
  \hline
    & 1 & 2 & 3 & 4 & 5 & 6
\end{tabular}
}
...
\begin{frame}
\frametitle{Ugens opgave, minimering i dRegAut java-pakken}
\begin{itemize}[<+->]
\item ”pseudo-kode”:\\
